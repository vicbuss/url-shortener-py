...
def with_retry(func: Callable[..., R]) -> Callable[..., R]:
	def wrapper(self: RedisClient, *args: object, **kwargs: object) -> R:
		...
		raise ConnectionError(
			f"Redis operation '{func.__name__}' failed after {self.max_retries} attempts"
		)
...
class RedisClient:
	...
	def set(
		...
		res: bool = self.client.set(name=key, value=value, ex=ex, exat=exat, nx=nx, xx=xx)
		return res
